...
static std::array<ImFont*, static_cast<size_t>( ImGui::ImFontId::Count )> sRegisteredFonts;
...
void ImGuiImpl::Initialise()
{
    ...
    RegisterDefaultFont();
    RegisterFont( ImGui::ImFontId::VeraMonoBold, "data/ui/fonts/ttf-bitstream-vera-1.10/VeraMoBd.ttf", 16.0f );
...
void ImGuiImpl::RegisterDefaultFont()
{
    RegisterFont( ImGui::ImFontId::Default, "data/ui/fonts/ttf-bitstream-vera-1.10/VeraMono.ttf", 16.0f );
}

void ImGuiImpl::RegisterFont( ImGui::ImFontId id, const std::filesystem::path& path, float size )
{
    SDL_assert( id != ImGui::ImFontId::Count );
...
ImFont* ImGuiImpl::GetFont( ImGui::ImFontId id )
{
    return sRegisteredFonts[ static_cast<size_t>( id ) ];
}
